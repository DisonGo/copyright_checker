...
double GetMatchedPercentage(std::vector<std::string>& ref, std::vector<std::string>& copy) {
    ...
    if (ref.size() > copy.size())
        all_count += ref.size() - copy.size();
...
void RemoveExtra(std::string& str) {
    ...
    for (size_t i = 0; i < str.size(); i++) {
        if (str[i] != ' ') result_string.push_back(str[i]);
        else if (result_string.empty()) continue;
        else if (i < str.size() - 1 && str[i + 1] != ' ') result_string.push_back(str[i]);
    }
    ...
    for (size_t i = 0; i < strings.size(); i++) {
        for (size_t j = 0; j < var_types.size(); j++) {
            ...
            if (find_result != std::string::npos) {
                ...
                for (; find_result < strings[i].size(); find_result++) {
                    char current = strings[i][find_result];
